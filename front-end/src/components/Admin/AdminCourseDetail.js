...
export default function AdminCourseDetail() {
  ...
  return (
    <div className="management-card" style={{ padding: "0" }}>
      <div style={{ padding: "2rem", borderBottom: "1px solid var(--border)", backgroundColor: "var(--bg-light)" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: "1rem" }}>
          ...
          <div style={{ textAlign: "right" }}>
             <span className="status-badge" style={{ backgroundColor: isCourseActive ? 'rgba(0,200,150,0.12)' : 'rgba(245,158,11,0.12)', color: isCourseActive ? 'var(--primary)' : '#f59e0b', fontSize: '0.9rem', padding: '8px 16px' }}>
      ...
      <div style={{ display: "flex", borderBottom: "1px solid var(--border)", padding: "0 2rem", backgroundColor: "#fff", flexWrap: "wrap" }}>
                ...
                {course.chapters.map((chapter, index) => {
                   ...
                   return (
                    <div key={chapterId} style={{ border: "1px solid var(--border)", borderRadius: "12px", overflow: "hidden" }}>
                      <div style={{ backgroundColor: "var(--bg-light)", padding: "1rem 1.5rem", display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: "1px solid var(--border)" }}>
                        <h4 style={{ margin: 0, fontSize: "1rem", fontWeight: 800, color: "var(--text-dark)" }}>Chương {index + 1}: {chapter.chapterName || chapter.title}</h4>
                        <span style={{ fontSize: "0.85rem", fontWeight: 700, color: "var(--primary)", backgroundColor: "rgba(0,200,150,0.1)", padding: "4px 10px", borderRadius: "99px" }}>{chapter.videos?.length || 0} Video</span>
                      </div>
                        ...
                        {chapter.videos && chapter.videos.length > 0 ? (
                          <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
                            {chapter.videos.map((vid, vIndex) => {
                              ...
                              return (
                              <div key={videoId} style={{ padding: "1rem", border: "1px solid #e2e8f0", borderRadius: "8px", backgroundColor: "#f8fafc" }}>
                ...
                {flashcards.map((fc) => {
                  ...
                  return (
                    <div key={fcId || Math.random()} style={{ border: "1px solid var(--border)", borderRadius: "12px", overflow: "hidden" }}>
                      ...
                      {isExpanded && (
                        <div style={{ borderTop: "1px solid var(--border)", backgroundColor: "var(--bg-light)", padding: "1.5rem" }}>
                          <h5 style={{ margin: "0 0 1rem 0", fontWeight: 800, fontSize: "0.95rem" }}>Danh sách từ vựng ({cardsList.length}):</h5>
                          {cardsList.length > 0 ? (
                            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: "1rem" }}>
                              {cardsList.map((card, idx) => {
                                // Ánh xạ đúng tên trường từ Backend C# trả về (frontText, backText, ipa, example)
                                ...
                                return (
                                <div key={card.itemID || card.id || idx} style={{ backgroundColor: "#fff", padding: "1.25rem", borderRadius: "12px", border: "1px solid #e2e8f0", boxShadow: "0 4px 6px rgba(0,0,0,0.02)" }}>
                            ...
                            <p style={{ margin: 0, fontStyle: "italic", color: "var(--text-muted)" }}>Bộ flashcard này chưa có từ vựng nào.</p>
                          )}
                        </div>
                      )}
                    </div>
                      ...
                      <div style={{ padding: "1.5rem", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        <div>
                          <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "0.5rem" }}>
                            ...
                            <span style={{ fontSize: "0.75rem", fontWeight: 800, backgroundColor: quiz.isActive ? 'rgba(0,200,150,0.1)' : 'rgba(245,158,11,0.1)', color: quiz.isActive ? 'var(--primary)' : '#f59e0b', padding: "2px 8px", borderRadius: "99px" }}>
                          ...
                          {groupsList.length > 0 ? (
                            <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
                              {groupsList.map((group, gIdx) => (
                                <div key={group.groupId || gIdx} style={{ backgroundColor: "#fff", padding: "1.5rem", borderRadius: "8px", border: "1px solid #e2e8f0" }}>
                                  ...
                                  {group.questions && group.questions.length > 0 ? (
                                        ...
                                        <div key={q.questionId || qIdx}>
                                          ...
                                          <div style={{ display: "flex", flexDirection: "column", gap: "6px", paddingLeft: "10px" }}>
                                            {answersList.map((ans, aIdx) => {
                                              ...
                                              return (
                                              <div key={ans.answerId || aIdx} style={{ display: "flex", alignItems: "center", gap: "8px", fontSize: "0.9rem", color: isCorrect ? "#059669" : "var(--text-body)", fontWeight: isCorrect ? 700 : 500, backgroundColor: isCorrect ? "#ecfdf5" : "transparent", padding: "6px 10px", borderRadius: "6px" }}>
                                                {isCorrect ? <strong style={{color: "#059669"}}>Đúng: </strong> : <span style={{color: "#cbd5e1"}}>Sai: </span>}
                                                {text}
                                              </div>
                                            )})}
                                          </div>
                                    ...
                                    <p style={{ margin: 0, fontStyle: "italic", color: "var(--text-muted)", fontSize: "0.9rem" }}>Nhóm này chưa có câu hỏi.</p>
                                  )}
                                </div>
                              ))}
                            </div>
                          ) : (
                            <p style={{ margin: 0, fontStyle: "italic", color: "var(--text-muted)" }}>Bài quiz này chưa có câu hỏi nào.</p>
                          )}
                        </div>
                      )}
                    </div>
