...
export function UserManagement() {
  ...
  const formatUserData = (u) => {
    return {
      ...
      joinedDate: u.createdAt || u.CreatedAt || u.createAt || u.CreateAt || new Date().toISOString(),
    };
  };
  ...
  const loadUsers = async () => {
    try {
      ...
      else if (response?.users) dataList = response.users;

      const mapped = dataList.map(formatUserData);
      setUsers(mapped);
    } catch (error) {
  ...
  const handleToggleStatus = (userId, isActive) => {
    Swal.fire({
      ...
      text: isActive ? "Người dùng này sẽ không thể đăng nhập nữa." : "Người dùng sẽ có thể truy cập bình thường.",
      icon: "warning",
      ...
      if (result.isConfirmed) {
        try {
          if (isActive) await lockUser(userId);
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      setUsers(dataList.map(formatUserData));
    } catch (error) {
  ...
  if (isLoading && users.length === 0) {
    return <div className="admin-loading-spinner"><div className="admin-spinner"></div><p>Đang tải dữ liệu...</p></div>;
  }
    ...
    <div className="management-card">
      ...
      <div className="management-header" style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
        ...
        <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap' }}>
          <select value={filterRole} onChange={(e) => setFilterRole(e.target.value)} className="form-input" style={{ width: 'auto', borderRadius: '99px', padding: '0.6rem 1rem' }}>
            <option value="all">Tất cả vai trò</option>
            <option value="admin">Admin</option>
            <option value="teacher">Giảng viên</option>
            <option value="student">Học viên</option>
          </select>
          <select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)} className="form-input" style={{ width: 'auto', borderRadius: '99px', padding: '0.6rem 1rem' }}>
            ...
            {filteredUsers.length > 0 ? (
              filteredUsers.map((user) => (
                <tr key={user.accountID}>
                  ...
                  <td className="fw-800" style={{ textAlign: 'center', color: user.role === 'ADMIN' ? '#ec4899' : user.role === 'TEACHER' ? '#00c896' : '#3b82f6' }}>
                    {user.role}
                  </td>
                  <td style={{ textAlign: 'center' }}>
                    <span className="status-badge" style={{ backgroundColor: user.isActive ? 'rgba(0,200,150,0.12)' : 'rgba(236,72,153,0.12)', color: user.isActive ? 'var(--primary)' : '#ec4899', display: 'inline-block' }}>
              ...
              <tr><td colSpan="4"><div className="admin-empty-data" style={{ padding: '3rem 0' }}>Không tìm thấy người dùng nào.</div></td></tr>
            )}
          </tbody>
        </table>
      </div>
