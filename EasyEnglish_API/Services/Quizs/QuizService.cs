...
{
    /// <summary>
    /// QuestionType convention:
    ///   1 = Multiple choice  → submit OptionId
    ///   2 = Fill in blank    → submit AnswerText, đáp án đúng trong MetaJson: {"answer":"..."} hoặc {"answers":["a","b"]}
    ///   3 = Essay            → submit AnswerText, GradedScore = null, giáo viên chấm tay
    /// </summary>
    public class QuizService : IQuizService
    {
        ...
        private static QuizDetailDto ToDetailDto(QuizAlias q) => new()
        {
            ...
            Groups = q.QuestionGroups.Select(g => new QuestionGroupDto
            {
                ...
                Questions = g.Questions.Select(qs => new QuestionDto
                {
                    ...
                    // Options chỉ trả về với trắc nghiệm (type 1)
                    // Fill-blank (2) và essay (3) trả về rỗng
        ...
        public async Task<SubmitQuizResponse> SubmitQuizAsync(int userId, int attemptId, SubmitQuizRequest req)
        {
            ...
            var gradableQuestions = questions.Where(q => q.QuestionType != 3).ToList();
            decimal perQuestion = gradableQuestions.Any() ? 100m / gradableQuestions.Count : 0;
            ...
            foreach (var ans in req.Answers)
            {
                ...
                switch (q.QuestionType)
                {
                    case 1: // Multiple choice
                        ...
                        gradedScore = isCorrect.Value ? perQuestion : 0;
                        break;
                ...
                if (gradedScore.HasValue)
                    totalScore += gradedScore.Value;
                ...
                // ── Build result detail ───────────────────────────────────────
                var resultDto = new QuestionResultDto
                {
                    QuestionId = q.QuestionId,
                    Content = q.Content,
                ...
                // Đáp án đúng — chỉ build khi sai hoặc essay để tiết kiệm payload
                if (isCorrect == false)
                {
                    if (q.QuestionType == 1)
                    {
                        // Trả về tất cả option đúng
                        resultDto.CorrectOptions = q.Options
                            .Where(o => o.IsCorrect)
                    ...
                    {
                        // Trả về đáp án text đúng từ MetaJson
                        try
                        {
                            using var doc = JsonDocument.Parse(q.MetaJson);
                            var root = doc.RootElement;
                            if (root.TryGetProperty("answer", out var single))
                                resultDto.CorrectAnswerText = single.GetString();
                            else if (root.TryGetProperty("answers", out var multi) && multi.ValueKind == JsonValueKind.Array)
                                resultDto.CorrectAnswerText = string.Join(" / ", multi.EnumerateArray().Select(a => a.GetString()));
                        }
                        catch { /* bỏ qua nếu MetaJson lỗi */ }
                    }
                }
            ...
            int correctCount = results.Count(r => r.IsCorrect == true);
            int wrongCount = results.Count(r => r.IsCorrect == false);
        ...
        {
            if (!optionId.HasValue) return false;
            return q.Options.Any(o => o.OptionId == optionId && o.IsCorrect);
        }

        private static bool GradeFillInBlank(Question q, string? answerText)
        {
            if (string.IsNullOrWhiteSpace(answerText)) return false;
            if (string.IsNullOrWhiteSpace(q.MetaJson)) return false;

            try
            {
                // MetaJson: {"answer":"correct"} hoặc {"answers":["a","b"]}
                using var doc = JsonDocument.Parse(q.MetaJson);
                var root = doc.RootElement;

                if (root.TryGetProperty("answer", out var single))
                    return string.Equals(answerText.Trim(), single.GetString()?.Trim(),
                        StringComparison.OrdinalIgnoreCase);

                if (root.TryGetProperty("answers", out var multi) &&
                    multi.ValueKind == JsonValueKind.Array)
                    return multi.EnumerateArray().Any(a =>
                        string.Equals(answerText.Trim(), a.GetString()?.Trim() ?? "",
                            StringComparison.OrdinalIgnoreCase));
            }
            catch { /* MetaJson lỗi → không chấm */ }
        ...
        private static void ValidateMetaJson(byte questionType, string? metaJson)
        {
            ...
            {
                throw new Exception("MetaJson không phải JSON hợp lệ.");
            }
        }
        ...
        public async Task<int> CreateQuestionAsync(int teacherId, int groupId, CreateQuestionRequest req)
        {
            ...
            return (await _repo.CreateQuestionAsync(groupId, question))!.QuestionId;
        }
        ...
        public async Task<int> CreateOptionAsync(int teacherId, int questionId, CreateOptionRequest req)
        {
            ...
            return (await _repo.CreateOptionAsync(questionId, option))!.OptionId;
        }
        ...
        public async Task<int> CreateAssetForGroupAsync(int teacherId, int groupId, CreateAssetRequest req)
        {
            ...
            return (await _repo.CreateAssetForGroupAsync(groupId, asset))!.AssetId;
        }

        public async Task<int> CreateAssetForQuestionAsync(int teacherId, int questionId, CreateAssetRequest req)
        {
            var asset = new Asset { AssetType = req.AssetType, Url = req.Url, ContentText = req.ContentText, Caption = req.Caption, MimeType = req.MimeType };
            return (await _repo.CreateAssetForQuestionAsync(questionId, asset))!.AssetId;
        }
        ...
        {
            var quiz = await _repo.GetQuizDetailAsync(quizId) ?? throw new Exception("Quiz not found");
            return ToDetailDto(quiz);
        }
